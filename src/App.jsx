...
export default function App() {
  const [form, setForm] = useState({
    ...
    adr: false, city_delivery: false,
    load: false, unload: false,       // ⬅ nieuw
    combi: false, km_levy: false      // ⬅ nieuw
  })
  ...
  const clearAll = () => {
    setForm({
      ...
      trailer_type:'tautliner',
      adr:false, city_delivery:false, load:false, unload:false, combi:false, km_levy:false
    })
    ...
    <div style={{minHeight:'100vh', padding:'24px', maxWidth:'900px', margin:'0 auto', fontFamily:'system-ui, sans-serif'}}>
      ...
      <form onSubmit={submit} style={{display:'grid', gap:'12px'}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'repeat(2, minmax(0, 1fr))', gap:'8px', alignItems:'center'}}>
          ...
          <label style={{display:'flex', gap:'6px', alignItems:'center'}}><input type="checkbox" name="load" checked={form.load} onChange={onChange}/> Laden</label>
          <label style={{display:'flex', gap:'6px', alignItems:'center'}}><input type="checkbox" name="unload" checked={form.unload} onChange={onChange}/> Lossen</label>
          <label style={{display:'flex', gap:'6px', alignItems:'center'}}><input type="checkbox" name="combi" checked={form.combi} onChange={onChange}/> Combi-transport (10% korting)</label>
          <label style={{display:'flex', gap:'6px', alignItems:'center'}}><input type="checkbox" name="km_levy" checked={form.km_levy} onChange={onChange}/> Kilometerheffing toepassen (€ 0,12/km)</label>
        </div>
      ...
      {quote && (
        <div style={{marginTop:'16px', border:'1px solid #e5e5e5', borderRadius:'12px', padding:'16px'}}>
          ...
          <p><strong>Berekend gewicht:</strong> {quote.derived.usage_ratio ? `${(quote.derived.usage_ratio*100).toFixed(0)}% vulling` : ''}</p>
