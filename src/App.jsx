...
export default function App() {
  const [form, setForm] = useState({ from:'', to:'', volume_m3:'', weight_kg:'', trailer_type:'tautliner', adr:false, city_delivery:false, toll:false, waiting_hours:'' })
  ...
  const submit = async (e)=>{
    e.preventDefault(); setLoading(true); setError(''); setQuote(null);
    try{
      const res = await fetch('/.netlify/functions/quote', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({
        from: form.from, to: form.to, volume_m3: Number(form.volume_m3||0), weight_kg: Number(form.weight_kg||0), trailer_type: form.trailer_type,
        options:{ adr:form.adr, city_delivery:form.city_delivery, toll:form.toll, waiting_hours: Number(form.waiting_hours||0) }
      })});
      const data = await res.json(); if(!res.ok) throw new Error(data.error||'Mislukt'); setQuote(data);
    }catch(err){ setError(String(err.message||err)) } finally { setLoading(false) }
  }
  return (<div style={{minHeight:'100vh', padding:'24px', maxWidth:'800px', margin:'0 auto', fontFamily:'system-ui, sans-serif'}}>
    <h1 style={{fontSize:'24px', fontWeight:700, marginBottom:'12px'}}>Transporttarief calculator</h1>
    <form onSubmit={submit} style={{display:'grid', gap:'12px'}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px'}}>
        <input name="volume_m3" type="number" step="0.01" placeholder="Volume (m³)" value={form.volume_m3} onChange={onChange} style={{padding:'10px', border:'1px solid #ccc', borderRadius:'8px'}} />
      ...
      <select name="trailer_type" value={form.trailer_type} onChange={onChange} style={{padding:'10px', border:'1px solid #ccc', borderRadius:'8px'}}>
        <option value="tautliner">Tautliner</option><option value="mega">Mega</option><option value="koel">Koel</option>
      </select>
      ...
      <button disabled={loading} style={{background:'#000', color:'#fff', padding:'10px 16px', borderRadius:'8px', border:'none', cursor:'pointer'}}>{loading ? 'Berekenen…' : 'Bereken tarief'}</button>
    </form>
    {error && <p style={{color:'#b00020', marginTop:'12px'}}>{error}</p>}
    {quote && (<div style={{marginTop:'16px', border:'1px solid #e5e5e5', borderRadius:'12px', padding:'16px'}}>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', columnGap:'24px', rowGap:'4px', marginTop:'8px'}}>
        {Object.entries(quote.breakdown).map(([k,v]) => (<div key={k} style={{display:'flex', justifyContent:'space-between'}}><span>{k}</span><span>€ {Number(v).toFixed(2)}</span></div>))}
      </div>
